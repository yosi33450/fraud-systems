...
export function FraudCommandCenter() {
  ...
  const [employeeDiscountActivity, setEmployeeDiscountActivity] = useState<EmployeeDiscountActivity>({ period: "30d", prefix: "", ordersChecked: 0, ordersWithAnyDiscountCode: 0, totalOrders: 0, totalAmount: 0, codes: [], recentUses: [] });
  ...
  const applySnapshot = (snapshot: DashboardSnapshot) => {
    ...
    setEmployeeDiscountActivity(snapshot.employeeDiscountActivity ?? { period: "30d", prefix: "", ordersChecked: 0, ordersWithAnyDiscountCode: 0, totalOrders: 0, totalAmount: 0, codes: [], recentUses: [] });
...
function EmployeesScreen({ employees, activity, settings, stores, onSync, syncing, syncProgress, onCreate, onUpdate, onSaveSettings }: { employees: Employee[]; activity: EmployeeDiscountActivity; settings: EmployeeMonitoringSettings; stores: Store[]; onSync: (storeId: string) => Promise<void>; syncing: boolean; syncProgress: string | null; onCreate: (input: Pick<Employee, "name" | "email" | "department"> & Partial<Pick<Employee, "privateEmail" | "address" | "couponCodes">>) => Promise<void>; onUpdate: (id: string, input: Pick<Employee, "name" | "email" | "department"> & Partial<Pick<Employee, "privateEmail" | "address" | "couponCodes">>) => Promise<Employee>; onSaveSettings: (input: EmployeeMonitoringSettings) => Promise<void> }) {
  ...
  const [couponPeriod, setCouponPeriod] = useState<"30d" | "all">("30d");
  const [allTimeActivity, setAllTimeActivity] = useState<EmployeeDiscountActivity | null>(null);
  const [allTimeLoading, setAllTimeLoading] = useState(false);
  const [allTimeError, setAllTimeError] = useState(false);
  const [couponFilter, setCouponFilter] = useState<"all" | "employee" | "assigned">("all");
  const [couponSort, setCouponSort] = useState<"usage" | "recent" | "code">("usage");
  ...
  useEffect(() => {
    if (couponPeriod !== "all") return;
    const controller = new AbortController();
    setAllTimeLoading(true);
    setAllTimeError(false);
    fetch(`/api/tenants/${tenantId}/employee-discount-activity`, { signal: controller.signal, cache: "no-store" })
      .then(async (response) => {
        if (!response.ok) throw new Error("ACTIVITY_LOAD_FAILED");
        return response.json() as Promise<EmployeeDiscountActivity>;
      })
      .then((result) => { if (!controller.signal.aborted) { setAllTimeActivity(result); setAllTimeLoading(false); } })
      .catch(() => { if (!controller.signal.aborted) { setAllTimeError(true); setAllTimeLoading(false); } });
    return () => controller.abort();
  }, [couponPeriod, activity.ordersChecked, activity.ordersWithAnyDiscountCode, activity.prefix]);
  ...
  const shownActivity = couponPeriod === "all" ? allTimeActivity : activity;
  const matchingCoupons = (shownActivity?.codes ?? []).filter((item) => item.code.includes(couponSearch.trim().toLowerCase())
    && (couponFilter === "all" || (couponFilter === "employee" && Boolean(activity.prefix && item.code.startsWith(activity.prefix.toLowerCase())))
      || (couponFilter === "assigned" && Boolean(item.assignedEmployeeId || item.assignmentConflict))));
  matchingCoupons.sort((a, b) => couponSort === "recent" ? Date.parse(b.lastUsedAt) - Date.parse(a.lastUsedAt) || a.code.localeCompare(b.code)
    : couponSort === "code" ? a.code.localeCompare(b.code) : b.orders - a.orders || a.code.localeCompare(b.code));
  const couponPageCount = Math.max(1, Math.ceil(matchingCoupons.length / couponPageSize));
  const currentCouponPage = Math.min(couponPage, couponPageCount);
  const visibleCoupons = matchingCoupons.slice((currentCouponPage - 1) * couponPageSize, currentCouponPage * couponPageSize);
  return <div className="page-content product-page employees-page"><PageHeading eyebrow="בדיקה פנימית" title="הטבות ורכישות עובדים" description="מעקב אחר קודי הנחה לעובדים, ההזמנות שבהן השתמשו בהם והתאמות לפרטי עובדים. קוד הנחה אינו גיפטקארד." action={<button className="primary-button" onClick={() => setAdding((value) => !value)}><Plus size={16} /> הוספת עובד לניטור</button>} />
    ...
    <section className="employee-discount-section" aria-labelledby="employee-discount-title"><div className="section-heading"><div><h2 id="employee-discount-title">שימוש בקודי הנחה</h2><span>קודים שהופיעו בהזמנות שנקלטו, כולל קודים שאינם של עובדים</span></div></div><div className="employee-discount-tools"><span>{shownActivity ? `${shownActivity.ordersWithAnyDiscountCode.toLocaleString("he-IL")} הזמנות עם קוד הנחה מתוך ${shownActivity.ordersChecked.toLocaleString("he-IL")} שנקלטו` : "טוען היסטוריית שימושים…"}</span>{couponPeriod === "all" && shownActivity?.earliestOrderAt ? <span>ההזמנה המוקדמת ביותר שנקלטה: {new Intl.DateTimeFormat("he-IL", { dateStyle: "short" }).format(new Date(shownActivity.earliestOrderAt))}</span> : null}</div>
      <div className="employee-coupon-filters">
        <label className="employee-coupon-select">תקופה<select aria-label="תקופת שימוש בקופונים" value={couponPeriod} onChange={(event) => { setCouponPeriod(event.target.value as typeof couponPeriod); setCouponPage(1); }}><option value="30d">30 ימים אחרונים</option><option value="all">כל ההיסטוריה שנקלטה</option></select></label>
        <label className="employee-coupon-search"><Search size={17} aria-hidden="true" /><input type="search" dir="ltr" aria-label="חיפוש קוד הנחה" placeholder="חיפוש קוד הנחה" value={couponSearch} onChange={(event) => { setCouponSearch(event.target.value); setCouponPage(1); }} /></label>
        <label className="employee-coupon-select">סינון<select aria-label="סינון קודי הנחה" value={couponFilter} onChange={(event) => { setCouponFilter(event.target.value as typeof couponFilter); setCouponPage(1); }}><option value="all">כל הקודים</option><option value="employee" disabled={!activity.prefix}>קודי עובדים{activity.prefix ? ` · ${activity.prefix.toUpperCase()}` : ""}</option><option value="assigned">משויכים לעובד</option></select></label>
        <label className="employee-coupon-select">מיון<select aria-label="מיון קודי הנחה" value={couponSort} onChange={(event) => { setCouponSort(event.target.value as typeof couponSort); setCouponPage(1); }}><option value="usage">הכי בשימוש</option><option value="recent">שימוש אחרון</option><option value="code">שם הקוד</option></select></label>
        <span className="employee-coupon-count">{matchingCoupons.length.toLocaleString("he-IL")} קודים</span>
      </div>
      {couponPeriod === "all" ? <p className="employee-coupon-coverage">“כל ההיסטוריה שנקלטה” כוללת רק הזמנות שכבר נשמרו במערכת; היא אינה משיכה מלאה של הזמנות ישנות מ־Shopify.</p> : null}
      {allTimeError && couponPeriod === "all" ? <p className="employee-discount-empty" role="alert">לא הצלחנו לעדכן את ההיסטוריה השמורה כרגע. אפשר לעבור ל־30 ימים ולנסות שוב.</p> : null}
      {allTimeLoading && !shownActivity ? <div className="employee-discount-empty" role="status">טוען היסטוריית שימושים…</div> : allTimeError && !shownActivity ? null : visibleCoupons.length ? <>
        <div className="employee-coupon-table-wrap"><table className="employee-coupon-table"><thead><tr><th scope="col">קוד הנחה</th><th scope="col">{couponPeriod === "all" ? "שימושים שנקלטו" : "שימושים ב־30 יום"}</th><th scope="col">שימוש אחרון</th><th scope="col">שיוך לעובד</th></tr></thead><tbody>{visibleCoupons.map((item) => { const owner = employees.find((employee) => employee.id === item.assignedEmployeeId); return <tr key={item.code}><td><strong dir="ltr">{item.code}</strong></td><td className="employee-coupon-number">{item.orders.toLocaleString("he-IL")}</td><td><time dateTime={item.lastUsedAt}>{new Intl.DateTimeFormat("he-IL", { dateStyle: "short" }).format(new Date(item.lastUsedAt))}</time></td><td>{owner ? owner.name : item.assignmentConflict ? "משויך לכמה עובדים" : <span className="employee-coupon-muted">לא משויך</span>}</td></tr>; })}</tbody></table></div>
        <div className="employee-coupon-pagination"><span>מציגים {((currentCouponPage - 1) * couponPageSize + 1).toLocaleString("he-IL")}–{Math.min(currentCouponPage * couponPageSize, matchingCoupons.length).toLocaleString("he-IL")} מתוך {matchingCoupons.length.toLocaleString("he-IL")}</span><div><label>שורות בעמוד<select aria-label="שורות בעמוד" value={couponPageSize} onChange={(event) => { setCouponPageSize(Number(event.target.value)); setCouponPage(1); }}><option value={10}>10</option><option value={25}>25</option><option value={50}>50</option></select></label><button type="button" disabled={currentCouponPage === 1} onClick={() => setCouponPage(currentCouponPage - 1)}>הקודם</button><span aria-live="polite">{currentCouponPage.toLocaleString("he-IL")} / {couponPageCount.toLocaleString("he-IL")}</span><button type="button" disabled={currentCouponPage === couponPageCount} onClick={() => setCouponPage(currentCouponPage + 1)}>הבא</button></div></div>
    ...
    <section className="employee-monitoring-settings"><div className="section-heading"><div><h2>הגדרות ניטור עובדים</h2><span>הגדרת תחילית לזיהוי קודי עובדים · אינה חיפוש בטבלת השימושים</span></div></div><div className="employee-prefix-setting"><label>תחילית קודי הנחה לעובדים<input dir="ltr" value={draft.couponPrefix} onChange={(event) => setDraft({ ...draft, couponPrefix: event.target.value })} placeholder="OVED" /></label><p>לדוגמה, <bdi>OVED</bdi> מזהה קודים כגון <bdi>OVED30</bdi>. החיפוש בקודים נמצא מעל הטבלה.</p><button className="primary-button" onClick={() => void onSaveSettings(draft).then(() => setSettingsNotice("הגדרות ניטור העובדים נשמרו.")).catch(() => setSettingsNotice("שמירת ההגדרות נכשלה."))}>שמור תחילית</button></div><details className="employee-secondary-rules"><summary>סריקה ורענון ידניים <ChevronDown size={16} /></summary><p className="context-note">הכלים האלה אינם נחוצים לחיפוש בטבלת השימושים. סריקת קודים שטרם נוצלו דורשת הרשאת Shopify מתאימה; רענון הזמנות של 30 יום עשוי לקחת כמה דקות.</p><label className="employee-manual-store">חנות לסריקה<select value={couponStoreId} onChange={(event) => setCouponStoreId(event.target.value)}>{stores.map((store) => <option key={store.id} value={store.id}>{store.name}</option>)}</select></label><div className="employee-settings-actions"><button className="secondary-button" onClick={() => void scanCoupons()} disabled={scanningCoupons}>{scanningCoupons ? "סורק…" : "אתר גם קודים שלא נוצלו"}</button><button className="secondary-button" onClick={() => couponStoreId && void onSync(couponStoreId)} disabled={!couponStoreId || syncing}><RefreshCcw size={15} /> {syncing ? "מרענן הזמנות…" : "רענן הזמנות מ־Shopify"}</button></div>{syncProgress ? <p role="status" className="sync-progress-note">{syncProgress}</p> : null}{availableCoupons.length ? <div className="coupon-results"><strong>{couponSource === "shopify" ? "קודים שנמצאו ב־Shopify" : "קודים שנקלטו בהזמנות"}</strong><p>{availableCoupons.slice(0, 40).join(" · ")}</p><small>{couponSource === "shopify" ? "קוד שקיים בחנות לא בהכרח שימש בהזמנה. שיוך לעובד נעשה בפרטי העובד." : "מוצגים רק קודים שכבר הופיעו בהזמנות שנקלטו."}</small></div> : null}</details>{settingsNotice ? <p role="status">{settingsNotice}</p> : null}<details className="employee-secondary-rules"><summary>בדיקות נוספות לעובדים <ChevronDown size={16} /></summary><div className="employee-rules-grid">
