...
function EmployeesScreen({ employees, activity, settings, stores, onSync, syncing, syncProgress, onCreate, onUpdate, onSaveSettings }: { employees: Employee[]; activity: EmployeeDiscountActivity; settings: EmployeeMonitoringSettings; stores: Store[]; onSync: (storeId: string) => Promise<void>; syncing: boolean; syncProgress: string | null; onCreate: (input: Pick<Employee, "name" | "email" | "department"> & Partial<Pick<Employee, "privateEmail" | "address" | "couponCodes">>) => Promise<void>; onUpdate: (id: string, input: Pick<Employee, "name" | "email" | "department"> & Partial<Pick<Employee, "privateEmail" | "address" | "couponCodes">>) => Promise<Employee>; onSaveSettings: (input: EmployeeMonitoringSettings) => Promise<void> }) {
  ...
  return <div className="page-content product-page employees-page"><PageHeading eyebrow="בדיקה פנימית" title="הטבות ורכישות עובדים" description="מעקב אחר קודי הנחה לעובדים, ההזמנות שבהן השתמשו בהם והתאמות לפרטי עובדים. קוד הנחה אינו גיפטקארד." action={<button className="primary-button" onClick={() => setAdding((value) => !value)}><Plus size={16} /> הוספת עובד לניטור</button>} />
    ...
    <section className="employee-discount-section" aria-labelledby="employee-discount-title"><div className="section-heading"><div><h2 id="employee-discount-title">שימוש בקודי ההנחה</h2><span>הזמנות Shopify שבהן הוזן קוד המתחיל ב־{activity.prefix ? <bdi>{activity.prefix.toUpperCase()}</bdi> : "תחילית שתגדיר"} · 30 ימים אחרונים</span></div><strong className="employee-discount-total">{formatCurrency(activity.totalAmount)} <small>סך הזמנות, לא גובה ההנחה</small></strong></div><div className="employee-discount-tools"><span>נבדקו {activity.ordersChecked.toLocaleString("he-IL")} הזמנות · {activity.ordersWithAnyDiscountCode.toLocaleString("he-IL")} עם קוד הנחה כלשהו</span><button className="secondary-button" onClick={() => couponStoreId && void onSync(couponStoreId)} disabled={!couponStoreId || syncing}><RefreshCcw size={15} /> {syncing ? "מרענן הזמנות…" : "רענן שימושים מ־Shopify"}</button></div>{syncProgress ? <p role="status" className="sync-progress-note">{syncProgress}</p> : null}
      {activity.codes.length ? <><div className="employee-code-list">{activity.codes.map((item) => { const owner = employees.find((employee) => employee.id === item.assignedEmployeeId); return <div className="employee-code-row" key={item.code}><strong dir="ltr">{item.code}</strong><span>{owner ? `משויך ל${owner.name}` : item.assignmentConflict ? "משויך ליותר מעובד אחד — יש לתקן" : "טרם שויך לעובד"}</span><span>{item.orders} הזמנות</span><bdi>{formatCurrency(item.amount)}</bdi></div>; })}</div><details className="employee-discount-orders"><summary>הצג הזמנות ושימושים אחרונים ({activity.recentUses.length}) <ChevronDown size={16} /></summary><div className="table-wrap"><table className="case-table"><thead><tr><th>תאריך</th><th>קוד הנחה</th><th>הזמנה</th><th>רוכש</th><th>קשר לעובד</th><th>סכום הזמנה</th></tr></thead><tbody>{activity.recentUses.map((use) => { const owner = employees.find((employee) => employee.id === use.assignedEmployeeId); return <tr key={`${use.storeId}:${use.orderId}:${use.code}`}><td>{new Intl.DateTimeFormat("he-IL", { dateStyle: "short", timeStyle: "short" }).format(new Date(use.createdAt))}</td><td dir="ltr">{use.code}</td><td>{use.orderNumber ?? `#${use.orderId.split("/").at(-1)}`}</td><td dir="ltr">{use.email || "לא התקבל"}</td><td>{use.assignmentConflict ? "קוד משויך לכמה עובדים" : owner ? use.buyerMatchesEmployee ? `תואם ל${owner.name}` : use.buyerIdentityAvailable ? `קוד של ${owner.name} · פרטי רוכש אחרים` : `קוד של ${owner.name} · אין פרטי רוכש לאימות` : "קוד לא משויך"}</td><td>{formatCurrency(use.amount)}</td></tr>; })}</tbody></table></div></details></> : <div className="employee-discount-empty"><strong>עדיין לא נמצאו הזמנות עם קודי הנחה מהתחילית הזו</strong><span>{activity.ordersChecked === 0 ? "עדיין לא נקלטו הזמנות ב־30 הימים האחרונים. אפשר לרענן אותן כעת מ־Shopify." : activity.ordersWithAnyDiscountCode === 0 ? "בהזמנות השמורות לא מופיעים קודי הנחה. רענן את ההזמנות כדי למשוך מחדש את השדה הזה מ־Shopify." : "נמצאו הזמנות עם קודי הנחה אחרים, אך לא עם התחילית הזו. בדוק את התחילית או רענן את ההזמנות."}</span></div>}
    </section>
